...
namespace statistics{
    ...
    public:
        ...
        void print_table_head(){
            std::cout << R"(\begin{tabular}{|c|ccc|ccc|}
                           \hline
                           \multirow{2}{*}{Methods} & \multicolumn{3}{c|}{Assembly Cost} & \multicolumn{3}{c|}{Time (s)} \\ \cline{2-7}
            ...
            for (int solverID = 0; solverID < folderNames.size(); solverID++)
            {
                ...
                for (int id = 0; id < filenames.size(); id++)
                {
                    ...
                    if (std::filesystem::exists(result_filename)){
                        ...
                        fin >> json_file;
                        fin.close();
                        compliances[solverID][id] = json_file["benchmark_compliance"].get<double>();
