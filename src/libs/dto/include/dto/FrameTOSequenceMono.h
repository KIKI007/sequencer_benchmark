...
#ifndef ROBO_CRAFT_FRAMETOSEQUENCEMono_H
#define ROBO_CRAFT_FRAMETOSEQUENCEMono_H
#include "FrameTO.h"
namespace dto
{
class FrameTOSequenceMono : public FrameTO
{
public:

    std::vector<Eigen::Vector2i> sections_;
    std::vector<double> sections_compliance_;

public:

    FrameTOSequenceMono(const frame::FrameAssembly &assembly): FrameTO(assembly)
    {
...
public:
    ...
    int nE_dynamic(){return end_partIDs_.size() - start_partIDs_.size();}

    int nS(){return sections_.size();}

    int nX() override{return nE_dynamic() * nS() + nS();}
    ...
    int nC() override{return nC_ascend() + nC_budget() + nC_mu();}

public:

    void setStartnEnd(std::vector<int> start_partIDs, std::vector<int> end_partIDs) override;

    void setSections(std::vector<int> sections, std::vector<double> sections_complaince);
...
#endif  //ROBO_CRAFT_FRAMETOSEQUENCEMono_H
